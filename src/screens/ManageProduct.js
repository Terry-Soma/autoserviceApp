...
export default function ManageProduct({ navigation, route }) {
  const product = route.params?.product;
  const isEditing = !!product;
  //  {"category": {"id": 1, "ner": "Гэрэл дохио"}, "categoryId": 1, "come_date": "2022-02-13", "id": 1, "img": null, "location": "Гаражны хойд тавиур", "ner": "95 урд гэрэл", "serNum": "123-123-22", "shirheg": 17, "une": 200000}
  const theme = useTheme();
  const [image, setImage] = useState(product.img);
  const [saving, setSaving] = useState(false)
  const [producName, setProductName] = useState(product.ner);
  const [productPrice, setProductPrice] = useState("" + product.une)
  const [productLoc, setProductLoc] = useState(product.location)
  const [productAmount, setProductAmount] = useState("" + product.shirheg);
  const [productCatId, setProductCatId] = useState(product.categoryId)
  const [productSerial, setProductSerial] = useState(product.serNum)
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#35495E" }}>
      <View
        style={{
          paddingVertical: 12,
      ...
      >
        <Text style={{ fontSize: 24, color: theme.colors.secondaryColor, textAlign: "center", fontFamily: fontVar.Man }}>
          Та барааны мэдээллээ өөрчлөнө уу
        </Text>
        ...
        >
          <ScrollView showsVerticalScrollIndicator={false} style={{
            ...
            >
              ...
              >Буцах</MyButton>
              <MyButton style={css.button} onPress={sendProductToServer}>Өөрчлөх</MyButton>
            </View>
          </ScrollView>
        </Animatable.View>)}
    </SafeAreaView>
